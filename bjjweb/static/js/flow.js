...
d3.json("render/", function(json) {
  ...
  */
  ...
  for (var i=0; i<data.links.length; ++i) {
    o = data.links[i];
    var get_index = function(pk){
        for (n in data.nodes){
            if (data.nodes[n].pk == pk)
                return n;
        }
    }
    o.source = data.nodes[get_index(o.source)];
    o.target = data.nodes[get_index(o.target)];
  }
